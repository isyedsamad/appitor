...
export async function incrementStudentCount(tx, adminDb, schoolId, branchId, amount, gender) {
    const analyticsRef = getAnalyticsRef(adminDb, schoolId, branchId);
    const branchRef = adminDb.collection("branches").doc(branchId);
    const schoolBranchRef = adminDb.collection("schools").doc(schoolId).collection("branches").doc(branchId);
    ...
    if (gender) {
        ...
        analyticsUpdates[genderKey] = FieldValue.increment(amount);
    }

    const branchUpdates = {
        studentCounter: FieldValue.increment(amount)
    };

    tx.set(analyticsRef, analyticsUpdates, { merge: true });
    tx.update(branchRef, branchUpdates);
    tx.update(schoolBranchRef, branchUpdates);
}
...
export async function incrementEmployeeCount(tx, adminDb, schoolId, branchId, amount) {
    ...
    const branchUpdates = {
        employeeCounter: FieldValue.increment(amount)
    };
...
export async function updateAttendanceAnalytics(tx, adminDb, schoolId, branchId, type, date, oldRecords = {}, newRecords = {}) {
    ...
    const isToday = date === todayStr.split('-').reverse().join('-');
