...
export default function SectionModal({open, onClose, onSuccess, classDataPage, sectionData}) {
  ...
  if (!open || !classDataPage) return null;
  async function handleSubmit() {
    if (!name.trim()) return;
    setLoading(true);
    try {
      await secureAxios.post("/api/school/academics/sections", {
        classId: classDataPage.id,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-md mx-4 modal-card">
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            <label className="text-sm text-muted">Student Capacity</label>
            <input
