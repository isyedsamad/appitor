...
const renderSingleReceipt = (doc, receipt, student, branchInfo, schoolUser, xOffset, yOffset, width, height, copyLabel) => {
    ...
    if (receipt.items && receipt.items.length > 0) {
        receipt.items.forEach(item => {
            const period = item.period && item.period.startsWith("one-time-")
                ? "One-Time"
                : (item.period || item.label || "");
            if (item.headsSnapshot && Array.isArray(item.headsSnapshot)) {
                const totalItemFee = item.headsSnapshot.reduce((s, h) => s + (h.amount || 0), 0);
                item.headsSnapshot.forEach(head => {
                    const headTotal = head.amount || 0;
                    const headPaid = totalItemFee > 0 ? (headTotal / totalItemFee) * (item.amount || 0) : 0;
                    tableData.push([
                        head.headName || "Fee Particular",
                        period,
                        `Rs. ${headTotal.toLocaleString()}`,
                        `Rs. ${Math.round(headPaid).toLocaleString()}`
                    ]);
                    payableTotal += headTotal;
                });
            } else {
                tableData.push([
    ...
    autoTable(doc, {
        ...
        head: [['Fee Particulars Description', 'Period', 'Total Fee', 'Amount Paid']],
        body: tableData,
        theme: 'striped',
        ...
        columnStyles: {
            0: { cellWidth: 'auto' },
            1: { cellWidth: isSmall ? 16 : 20, halign: 'left' },
            2: { halign: 'right', cellWidth: isSmall ? 20 : 25 },
            3: { halign: 'right', fontStyle: 'bold', cellWidth: isSmall ? 22 : 28 }
        },
        didParseCell: (data) => {
