...
export const generateReceiptPDF = ({ receipt, student, branchInfo, options = { size: 'a4', copies: 1 } }) => {
    ...
    for (let i = 0; i < numCopies; i++) {
        ...
        renderSingleReceipt(doc, receipt, student, branchInfo, xOffset, yOffset, receiptWidth, receiptHeight, copyLabel);
    }
...
const renderSingleReceipt = (doc, receipt, student, branchInfo, xOffset, yOffset, width, height, copyLabel) => {
    ...
    // doc.setFillColor(148, 146, 146);
    doc.rect(xOffset + margin, currentY - 2, contentWidth, headerH, 'F');

    currentY += 2;
    doc.setFont("helvetica", "bold");
    doc.setFontSize(isVerySmall ? 9 : (isSmall ? 11 : 14));
    doc.setTextColor(255, 255, 255);
    doc.text(branchInfo?.name?.toUpperCase() || "SCHOOL NAME", centerX, currentY + (isVerySmall ? 2 : 4), { align: 'center' });
    ...
    if (!isVerySmall) {
        ...
        doc.text(`Branch Code: ${branchInfo?.branchCode || "N/A"}`, centerX, currentY, { align: 'center' });
    }

    if (!isSmall) {
        currentY += 4;
        doc.text(`Session: ${receipt.sessionId || "N/A"}`, centerX, currentY, { align: 'center' });
    }
    ...
    sY += isSmall ? 4 : 5;
    doc.setFillColor(249, 250, 251);
    doc.rect(sX - 1, sY, summaryW + 1, isSmall ? 5 : 6, 'F');
    doc.setTextColor(17, 24, 39);
    doc.setFont("helvetica", "bold");
    doc.text("PAID AMOUNT:", sX, sY + (isSmall ? 3.5 : 4.5));
    const dispAmount = receipt.paidAmount || receipt.amount || 0;
    doc.text(`Rs. ${dispAmount.toLocaleString()}`, xOffset + width - margin - 2, sY + (isSmall ? 3.5 : 4.5), { align: 'right' });
    ...
    doc.text("BALANCE DUE:", sX, sY + (isSmall ? 4 : 4.5));
    doc.text(`Rs. ${(receipt.newBalance || 0).toLocaleString()}`, xOffset + width - margin - 2, sY + (isSmall ? 4 : 4.5), { align: 'right' });
