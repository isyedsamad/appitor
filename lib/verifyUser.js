...
export async function verifyUser(req, requiredPermission, isForAll = false, targetBranch = null) {
  ...
  if (targetBranch && user.currentBranch && user.currentBranch !== "*" && user.currentBranch !== targetBranch) {
  ...
  const roleStudent = {
    permissions: ['leave.manage', 'complaint.manage']
  };
  var roleData;
